...
# Emprise
EMPRISE_VIDE_MAX    = 0.05
EMPRISE_SOUS_MIN    = 0.05
...
for i in range(1, 4):
    try:
        ...
        # Filtrer les toutes petites géométries (< 2m² = artefacts)
        bati_pci = bati_pci[bati_pci['bat_area'] > 2].copy()
...
while True:
    ...
    if len(bloc) < MAX_REQ: break
    offset += MAX_REQ
    time.sleep(0.3)

bat_topo = gpd.GeoDataFrame.from_features(features, crs='EPSG:2154')
col_h    = next((c for c in bat_topo.columns
                 if c.lower() in ('hauteur','height','z_max','altitude_maximale_toit')), None)
...
def charger_parcelles(code, essais=5, delai=15):
    ...
    for i in range(1, essais+1):
        try:
            ...
            if len(gdf) == 0: raise ValueError('0 parcelles')
...
# FIX : remplace pd.read_parquet complet par filtre PyArrow
...
try:
    ...
    if c_sec and c_num:
        m['cle']   = (m[c_sec].astype(str).str.strip() + '_' +
...
# Emprise au sol — FIX : sjoin centroïde (10x moins de RAM que overlay)
print('Calcul emprise au sol...')

# Emprise PCI par parcelle
...
emprise = joined.groupby('cle').agg(
    emprise_m2 = ('bat_area','sum'),
    nb_bat_pci = ('bat_area','count')
).reset_index()
del pci_pts, joined; gc.collect()
...
del bat_pts, h_join; gc.collect()

parcelles = parcelles.merge(emprise, on='cle', how='left')
...
parcelles['emprise_ratio'] = parcelles['emprise_m2'] / parcelles['parc_area']
del emprise, h_agg; gc.collect()

print(f'✅ Emprise calculée via sjoin')
print(f'   Vides (<5%)       : {(parcelles["emprise_ratio"] < EMPRISE_VIDE_MAX).sum()}')
print(f'   Sous-exploités    : {parcelles["emprise_ratio"].between(EMPRISE_SOUS_MIN, EMPRISE_SOUS_MAX).sum()}')
...
# Géolocalisation
col_pt = next((c for c in friches.columns
               if friches[c].astype(str).str.contains('POINT', na=False).any()), None)
...
# DVF 2014–2023
# Colonnes chargées :
# - id_parcelle    → jointure cadastre
# - date_mutation  → date dernière transaction
# - nature_mutation → Vente / Donation / Succession
# - nature_culture  → Maison / Terrain / Appartement...
...
for annee in range(ANNEE_DVF_DEBUT, ANNEE_DVF_FIN + 1):
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            # Garder seulement maisons et terrains (exclure appartements)
            if 'nature_culture' in df.columns:
                df = df[
                    df['nature_culture'].isin(NATURES_OK) |
                    df['nature_culture'].isna()
                ].copy()
            # Garder mutations significatives
...
if dvf_frames:
    ...
    # Parcelles avec au moins 1 mutation
    cles_avec_mutation = set(dvf['cle'].unique())

    # Dernière mutation par parcelle (pour affichage popup)
...
# Catégorie 1 : Terrains vides
vides = parc_pp[parc_pp['emprise_ratio'] < EMPRISE_VIDE_MAX].copy()
vides['categorie']       = 'Terrain vide'
vides['hauteur']         = 0.0
vides['ecart_max']       = 0.0
vides['haut_voisin_max'] = 0.0
print(f'  🔵 {len(vides)} terrains vides')

# Catégorie 2 : Sous-exploités
sous = parc_pp[
...
print(f'  🟣 {len(sous)} sous-exploités')
...
dc_parc = dc_parc.sort_values('ecart_max', ascending=False)\
                  .drop_duplicates(subset='cle', keep='first').copy()
...
if MAJIC_OK: candidats = candidats[~candidats['cle'].isin(cles_copro)].copy()
if cles_avec_mutation: candidats = candidats[~candidats['cle'].isin(cles_avec_mutation)].copy()
candidats = candidats[
...
def geocoder_df(df):
    if len(df) == 0:
        df = df.copy(); df['adresse'] = []; return df
    pts = df.copy()
    if pts.crs and pts.crs.to_epsg() != 4326: pts = pts.to_crs(epsg=4326)
    pts['geometry'] = pts.geometry.centroid
    adrs = []
    for _, row in pts.iterrows():
        adrs.append(get_adresse(round(row.geometry.y,6), round(row.geometry.x,6)))
        time.sleep(0.1)
    df = df.copy(); df['adresse'] = adrs; return df
...
if len(gdf_friches) > 0: gdf_friches = geocoder_df(gdf_friches)
...
def type_prop(p, s):
    if p in ('Particulier','Inconnu','N/A',''): return 'Particulier'
    return 'Société'
...
fg_vid = folium.FeatureGroup(name=f'🔵 Terrains vides ({len(vides)})',      show=True)
fg_sou = folium.FeatureGroup(name=f'🟣 Sous-exploités ({len(sous)})',       show=True)
...
def ajouter(fg, gp, lat, lon, coul, col_f, icone, html, tip, sec, num):
    ...
    folium.GeoJson(geom_wgs.__geo_interface__,
        style_function=lambda x, c=coul: {'color':c,'weight':2,'fillOpacity':0.4}
    ).add_to(fg)
    folium.Marker([lat, lon], popup=folium.Popup(html, max_width=320),
        tooltip=tip, icon=folium.Icon(color=col_f, icon=icone, prefix='fa')
    ).add_to(fg)
...
for rang,(_, row) in enumerate(dc_parc.iterrows(),1):
    try:
        sec=str(row.get('section','')).strip(); num=str(row.get('numero','')).strip()
        surf=int(row['contenance']) if pd.notna(row.get('contenance')) else '?'
        adr=str(row.get('adresse','') or 'Adresse inconnue')
        prop=str(row.get('denomination','Particulier')); sir=str(row.get('siren','') or '')
        ...
        html=(f"<div style='font-family:Arial;font-size:13px;min-width:270px;line-height:1.9'>"
    ...
    except Exception as e: print(f'  ⚠️ DC {rang}: {e}')
...
for rang,(_, row) in enumerate(vides.iterrows(),1):
    try:
        ...
        html=(f"<div style='font-family:Arial;font-size:13px;min-width:270px;line-height:1.9'>"
              f"<b style='font-size:15px;color:{COUL_VIDE}'>#{rang} Terrain vide</b><br>"
              f"<a href='{gm}' target='_blank' style='color:#1a6fb5;font-weight:bold;text-decoration:none'>📍 {adr}</a><br><br>"
              f"<b>Parcelle :</b> {sec} n°{num} | <b>Surface :</b> {surf} m²<br>"
              f"<b>Emprise PCI :</b> {emp}%"
              f"<hr style='margin:5px 0'><b>Proprio :</b> {prop} ({type_prop(prop,sir)})<br>{sir_h}"
              f"<hr style='margin:5px 0'><a href='{ge}' target='_blank' style='background:#1a73e8;color:white;padding:5px 12px;border-radius:6px;text-decoration:none;font-size:12px'>🌍 Google Earth</a></div>")
        ajouter(fg_vid,gp,lat,lon,COUL_VIDE,'blue','tint',html,f'#{rang} Vide | {adr}',sec,num)
    except Exception as e: print(f'  ⚠️ Vide {rang}: {e}')
...
for rang,(_, row) in enumerate(sous.iterrows(),1):
    try:
        ...
        html=(f"<div style='font-family:Arial;font-size:13px;min-width:270px;line-height:1.9'>"
              f"<b style='font-size:15px;color:{COUL_SOUS}'>#{rang} Sous-exploité</b><br>"
              f"<a href='{gm}' target='_blank' style='color:#1a6fb5;font-weight:bold;text-decoration:none'>📍 {adr}</a><br><br>"
              f"<b>Parcelle :</b> {sec} n°{num} | <b>Surface :</b> {surf} m²<br>"
              f"<b>Emprise :</b> {emp}% | <b>Haut. min :</b> {haut}m"
              f"<hr style='margin:5px 0'><b>Proprio :</b> {prop} ({type_prop(prop,sir)})<br>{sir_h}"
              f"<hr style='margin:5px 0'><a href='{ge}' target='_blank' style='background:#1a73e8;color:white;padding:5px 12px;border-radius:6px;text-decoration:none;font-size:12px'>🌍 Google Earth</a></div>")
        ajouter(fg_sou,gp,lat,lon,COUL_SOUS,'purple','building',html,f'#{rang} Sous-exp. | {adr}',sec,num)
    except Exception as e: print(f'  ⚠️ Sous {rang}: {e}')
...
for rang,(_, row) in enumerate(gdf_friches.iterrows(),1):
    try:
        ...
        geom=row.geometry; c=geom.centroid if geom.geom_type!='Point' else geom
    ...
    except Exception as e: print(f'  ⚠️ Friche {rang}: {e}')
...
for rang,(_, row) in enumerate(parc_vacants.iterrows(),1):
    try:
        ...
        html=(f"<div style='font-family:Arial;font-size:13px;min-width:270px;line-height:1.9'>"
              ...
              f"<hr style='margin:5px 0'><b>Proprio :</b> {prop} ({type_prop(prop,sir)})<br>{sir_h}"
              f"<hr style='margin:5px 0'>✓ Pleine propriété<br>✓ Aucune vente DVF depuis {ANNEE_DVF_DEBUT}<br>✓ Emprise faible ({emp}%)"
    ...
    except Exception as e: print(f'  ⚠️ Vacant {rang}: {e}')

for fg in [fg_dc,fg_vid,fg_sou,fg_fri,fg_vac]: fg.add_to(carte)
...
carte.get_root().html.add_child(folium.Element(
    ...
    f"box-shadow:0 2px 10px rgba(0,0,0,.2);font-family:Arial;font-size:12px;min-width:220px'>"
    ...
    f"<span style='color:{COUL_VIDE}'>&#9679;</span> Terrains vides {len(vides)}<br>"
    f"<span style='color:{COUL_SOUS}'>&#9679;</span> Sous-exploités {len(sous)}<br>"
